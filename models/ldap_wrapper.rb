...
class LdapWrapper
  ...
  def initialize
    @ldap = Net::LDAP.new(:host => "10.110.30.10", :port => "636", :encryption => { method: :simple_tls, tls_options:  { verify_mode: OpenSSL::SSL::VERIFY_NONE } }, :base => BASE, :auth => { :method => :simple, :username => ENV.fetch('AD_USER'), :password => ENV.fetch('AD_PASS') })
  end
